...
- Never promise that this skill can make every project production-ready or keep one production-ready forever. The current package intentionally cannot issue a production-ready result: it has no provider-specific, pre-provisioned asymmetric production-authority verifier and always fails `certify` closed with `CERT015` after local candidate checks. A future successful claim would still be limited to the inspected repository, one trusted source commit and its clean direct-child attestation commit, one named environment, one evidence window, and the authorities actually exercised.
...
- For fully adopted or production-ready requests, use the explicitly authorized convergence loop in [certification-and-convergence.md](references/certification-and-convergence.md). Continue through project-specific implementation and candidate evidence capture; then report the external-verifier blocker literally. Do not stop at installation or scaffolding, and do not suppress `CERT015`.
- For an existing production-ready claim, validate its trusted source and direct-child attestation commits, v2 candidate-integrity records and freshness, project-native maintenance gate, claimed production authority, and rollback path. The bundled verifier still cannot endorse the claim; treat `CERT015`, drift, or expiry as a failed claim, never historical production proof.
...
7. Verify and report literally.
   ...
   - For a production-ready request, read [certification-and-convergence.md](references/certification-and-convergence.md). Commit the implementation as source commit `S`, create HMAC-consistent v2 candidate records bound to concrete repository and deployment-target identities, and create one direct-child attestation commit `A` containing only certification, coverage, and referenced records. Obtain current `A` from trusted source-control or CI context and run `python3 <skill-dir>/scripts/harness.py certify --root <repo> --profile <adaptive|standard|full> --commit <trusted-attestation-commit-A> --attestation-key-file <absolute-candidate-integrity-key-path>`. Even when every local check passes, report the expected nonzero `CERT015` blocker. The caller-selected HMAC key is not production authority, and the current package cannot say `production-ready`.
...
- Create or resume an ExecPlan, then iterate `audit -> implement -> exercise -> candidate evidence -> project-native gate -> certify` until every local check is resolved and `CERT015` identifies the unavailable external verifier, or another concrete authority blocks progress. Do not return a production-ready outcome.
...
- Bind every candidate record and the manifest to one trusted source commit plus concrete repository and deployment-target identities, bind the check to its direct-child attestation commit and production environment, expire it within seven days, and rerun convergence after relevant changes. These controls preserve candidate integrity; they do not replace the unavailable asymmetric external authority verifier.
...
- Add `--allow-non-git` to non-certification operations only when the user intentionally places a non-Git project in scope; never infer a broad root. The candidate-integrity `certify` operation requires the Git source/attestation commit boundary, and the bundled package cannot return a production-ready result.
...
- Use the configured coverage authority (bundled default: `docs/agent-harness/coverage-matrix.md`) as the inventory gate: every row must be `verified`, `candidate`, `blocked`, or a justified `N/A`. Treat local adoption as structurally complete only when every applicable row is `verified`, every inapplicable row has a justified `N/A`, and every completed status links to one fresh HMAC-consistent v2 candidate record; `candidate` and `blocked` remain explicit incomplete states. None of these states overrides `CERT015`.
...
- Use [certification-and-convergence.md](references/certification-and-convergence.md) for authority-bounded convergence, HMAC candidate-integrity records, continuous invalidation, source/attestation binding, and the unavailable external-verifier blocker.
