...
description: Audit, adopt, repair, converge, continuously maintain, or evaluate an explicit OpenAI-style repository harness with concise AGENTS.md routing, versioned knowledge, managed ExecPlans, verification and observability contracts, mechanical guardrails, optional model-and-agent runtime/eval contracts, production evidence, and entropy maintenance. Use for requests that explicitly mention harness engineering, an agent-first or agent-readable repository, repository knowledge maps, docs/agent-harness, model/provider/tool/MCP runtime contracts, agent evaluations, production-ready harness adoption, continuous harness maintenance, managed docs/exec-plans lifecycle, or authoring, executing, resuming, migrating, and completing an ExecPlan under that lifecycle. Do not use for ordinary feature work, one-off informal planning, generic CI/testing, generic documentation cleanup, or code review unless the user also asks to establish or use the repository harness.
---
...
- Certification is not used by default. The native project gate and literal local evidence are the normal completion proof. The explicit read-only `certify` command is available only as an optional strict overlay for a user or repository policy that asks for `harness-ready`/`CERT000`; it does not claim that the product was deployed. Add `--require-production-attestation` only when the user explicitly asks for provider-backed production attestation; without such an integration that stricter check fails closed with `CERT015`.
...
- For fully adopted harness requests, use the explicitly authorized convergence loop in [certification-and-convergence.md](references/certification-and-convergence.md). Continue through project-specific implementation, evidence capture, the project-native gate, declared invalidation mode, and recovery; do not stop at installation or scaffolding. Enter the strict attestation loop only when `harness-ready`, `CERT000`, or provider-backed production evidence is explicitly required.
...
- Treat the repository as a fresh adoption only when it has no explicit harness configuration, harness documentation tree, coverage manifest, evidence records, registered project-native harness checker, repository-local harness package, or mapped equivalent authority. An absent certification manifest is normal because strict certification is optional and disabled by default.
...
3. Design a proportional target.
   ...
   - When the target invokes an AI runtime, read the target templates for [model-and-agent runtime](assets/templates/project/docs/agent-harness/model-and-agent-runtime.md) and [agent evaluations](assets/templates/project/docs/agent-harness/evals.md). Keep both authorities `N/A` for non-AI repositories.
...
6. Close the feedback loop.
   ...
   - Register repository-local scripts or skills and authorized source-control and review entry points so agents can gather evidence directly instead of relying on copied chat context. Register CI context only when it already exists or the user explicitly requests CI automation.
   - Register model/provider/snapshot choices, tool/MCP permissions, state/resume behavior, and representative evaluation commands when an AI runtime is applicable. Include side effects, retry safety, error behavior, and measured quality/latency/token/cost signals.
...
7. Verify and report literally.
   - Run project-native checks and `python3 <skill-dir>/scripts/harness.py check --root <repo>`. Any adoption-complete claim must add `--warnings-as-errors`: use `--profile <standard|full>` for an adopted canonical profile, or adaptive checking with explicitly mapped authorities for a custom shape. A custom shape also requires its project-native checker and a mapped coverage matrix that retains every bundled capability inventory row; paths may vary, capability coverage may not be silently omitted. Never infer full adoption from an adaptive zero-error discovery report alone. A native gate is the default completion proof; strict attestation is an opt-in overlay.
   - Distinguish `verified locally`, `not run`, `blocked`, `certification not used` (the default), `harness-ready`, `release pending`, and an actually observed production deployment.
...
- Create or resume an ExecPlan, then iterate `audit -> implement -> exercise -> evidence -> project-native gate`. Add `-> strict attestation -> certify` only when the explicit request requires a current `harness-ready`/`CERT000` result or provider-backed production evidence.
- Implement the durable checker inside the target repository using its native language and tooling. Default maintenance to the repository-local `manual` mode: the agent runs the native gate before completion and records fresh evidence. Enable strict attestation maintenance only when that profile is explicitly requested. Do not create or modify GitHub Actions, GitLab CI, or other hosted workflow files unless the user explicitly requests CI automation, regardless of repository visibility. Preserve unrelated existing workflows. When CI automation is explicitly requested, wire the native gate to pull requests, pushes, and a bounded schedule. The bundled helper remains an independent read-only bootstrap and cross-check; any target CI must not depend on the skill installation path.
- Repair safe detected drift only when the current user request or an explicitly authorized repository-native maintenance trigger permits it. Installation itself creates no trigger. Fail closed and expose the exact blocker when remediation needs a secret, human approval, destructive action, external write, merge, release, deployment, production access, or product judgment.
- When strict attestation is enabled, bind every evidence record and the manifest to one trusted source commit plus concrete repository and harness-target identities, bind the check to its direct-child attestation commit and named evidence environment, expire it within seven days, and rerun convergence after relevant changes. Use production-scoped evidence only for capabilities actually exercised there.
...
│   ├── model-and-agent-runtime.md (when applicable)
│   ├── evals.md (when applicable)
│   ├── output-contract.md
│   ├── verification-matrix.md
│   ├── entropy-cleanup-checklist.md
│   ├── coverage-matrix.md
│   ├── certification.md (optional strict overlay)
│   ├── certification.json (optional strict overlay)
│   └── evidence/
├── exec-plans/
│   ├── index.md
│   ├── plan-template.md
...
- Do not add CI gates unless the user explicitly requests CI automation and the underlying command is deterministic and reasonably fast. Introduce high-noise checks in report-only mode, then enforce after the baseline is clean.
...
- Treat the bundled script as explicitly invoked assessment, adoption, and maintenance cross-check tooling. Implement and register a project-native durable checker before claiming mechanical enforcement. Claim CI enforcement only when CI already exists or the user explicitly requested it, and never make repository CI depend on the skill installation path.
...
- Use the configured coverage authority (bundled default: `docs/agent-harness/coverage-matrix.md`) as the inventory gate: every row must be `verified`, `candidate`, `blocked`, or a justified `N/A`. Treat native adoption as complete only when every applicable local requirement is verified and every inapplicable requirement has a justified `N/A`. When strict attestation is enabled, every completed status must additionally link to one fresh HMAC-consistent v2 evidence record; `candidate` and `blocked` remain explicit incomplete states.
...
- Use the target [model-and-agent runtime contract](assets/templates/project/docs/agent-harness/model-and-agent-runtime.md) for applicable model, state, tool/MCP, safety, and multi-agent decisions.
- Use the target [agent evaluation contract](assets/templates/project/docs/agent-harness/evals.md) for representative task baselines and model/orchestration comparisons.
- Use [certification-and-convergence.md](references/certification-and-convergence.md) for authority-bounded convergence, HMAC evidence-integrity records, manual or opt-in automated revalidation, source/attestation binding, harness-ready certification, and the optional production-attestation profile.
