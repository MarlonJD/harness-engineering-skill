...
Strict certification is not used by default. The native project gate and local evidence are the completion proof. Add the optional `certification.md`, `certification.json`, and evidence overlay only when a user or repository policy explicitly requests a bounded `CERT000` result.
...
| Run strict harness attestation | Optional `certification.md` and `certification.json` | Not used by default. Add the optional overlay only when a bounded `harness-ready`/`CERT000` result is explicitly required; add `--require-production-attestation` only when independent production evidence is explicitly in scope |
