...
Installing this skill creates no watcher, CI job, schedule, or repair loop. After an explicit repository-scoped `$apply-harness-engineering` invocation, a deliberately implemented project-native harness gate may run on pull requests, pushes, and a bounded schedule. Bind HMAC-consistent v2 evidence records and the `harness-ready` manifest to source commit `S`, and verify them only from its clean direct-child attestation commit `A`. Expire certification within seven days or sooner, and immediately invalidate it when `HEAD` moves past `A` or a required command, coverage row, record, or declared applicability or authority input changes or fails. These local records support bounded repository-harness certification but do not authenticate production authority. When all ordinary checks pass, the bundled verifier returns `CERT000`.
