...
## Maintain candidate integrity and block unsupported production claims

Installing this skill creates no watcher, CI job, schedule, or repair loop. After an explicit repository-scoped `$apply-harness-engineering` invocation, a deliberately implemented project-native candidate gate may run on pull requests, pushes, and a bounded schedule. Bind HMAC-consistent v2 candidate records and the `candidate-only` manifest to source commit `S`, and verify them only from its clean direct-child attestation commit `A`. Expire the candidate within seven days or sooner, and immediately invalidate it when `HEAD` moves past `A` or a required command, coverage row, record, or declared approval or rollback input changes or fails. These local records do not authenticate production authority, and the bundled verifier must still return nonzero `CERT015`.

Only an explicitly authorized repository-native trigger may repair safe repository-local drift within existing authority. It must fail closed and escalate when repair requires secrets, destructive actions, external writes, merge, release, deployment, production access, or product judgment. Refreshing and rechecking the candidate can prove local recovery; it cannot remove `CERT015` or establish production readiness. A future production verifier must use an independently provisioned provider trust root and verify repository identity, deployment target, issuer, approval, rollback, artifact provenance, freshness, and revocation.
